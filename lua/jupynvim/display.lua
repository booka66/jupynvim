...
function M.render_notebook(notebook)
  ...
  vim.g.current_notebook = notebook -- Make it globally accessible
  ...
  if not notebook.cells or type(notebook.cells) ~= "table" then
    vim.notify("Invalid notebook format: no cells found", vim.log.levels.ERROR)
    vim.api.nvim_buf_set_lines(notebook_buffer, 0, 1, false, {"-- Invalid notebook format: no cells found --"})
    return
  end
  
  for i, cell in ipairs(notebook.cells) do
    -- Safety check for cell structure
    if not cell or type(cell) ~= "table" or not cell.cell_type then
      vim.notify("Invalid cell format at index " .. i, vim.log.levels.WARN)
      goto continue
    end
    ...
    -- Add cell content (with nil safety)
    local content = parser.get_cell_content(cell) or ""
    local content_lines = {}
    
    -- Only try to split if content is not empty
